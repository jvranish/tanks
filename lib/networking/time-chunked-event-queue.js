...
events that they have received from the server.
...
However, in order to draw progress (and especially to do it smoothly) of the
missile zooming towards its target, we need to continually update the position
of the missile, and we will need to do it even if there are no new incoming
player events. Additionally, when you're animating something like this, you
don't have control how fast time progresses, you have to render frames at
consistent intervals to make the animation smooth. if you are using this for
games, you'll likely use javascript's `requestAnimationFrame()` to know when to
render the next frame, which means you get _told_ what time it really is and
you'll have to run the simulation forward to the appropriate point.
...
This means that we can't just process events as we receive them, we have to
buffer them up and process them in chunks based on the amount of time that has
passed.
...
yes you can, and yes it will, but this will lead to stuttering, because you are
not _guaranteed_ to get another chunk on time after that one.
...
- There also is the unit of time chunking you want to process incoming events in
  (which everyone needs to agree on). This will define your "simulation" frame
  rate.
...
/**
 * @template E
 * @typedef {{ simTime: number; dt: number; peerEvents: E[]; }} TimeChunk
 */

/** @template {{ msgTime: number }} E */
export class TimeChunkedEventQueue {
  /**
   * @param {{ simTime: number; tickPeriodMs: number; timeChunkMs: number; now:
   *   number; }} init
   */
  constructor({ simTime, tickPeriodMs, timeChunkMs, now }) {
    ...
    /**
     * @type {{ simTime: number; dt: number; peerEvents: E[];
     * }[]}
     */
  ...
  processTick(msgTime) {
    // Local simTime is always incremented timeChunkMs at a time. Local simTime
    // is always <= the msgTime from the last tick. If there is some time (and
    // events) still left to process, we will leave them in the queue and
    // process them in the next tick.
    for (
  ...
  getEvents(time = performance.now()) {
    ...
    // Only advance time if we have chunks in the queue (we should normally have
    // _some_ chunks even if they are empty, and if not, we need to slow down
    // time)
    if (this.eventChunkQueue.length > 0) {
      ...
      if (this.localTime + this.tickPeriodMs < this.lastChunkEndTime) {
        // If we're more than a tick behind, skip ahead till we're only one tick
        // behind.
    ...
    // After processing events, if the hasEnded flag is set and the event queue
    // is empty, return null to indicate that there will be no more future
    // events.
    if (
      this.hasEnded &&
      this.eventChunkQueue.length === 0 &&
...
/**
 * Shifts elements off the beginning of the array while the predicate is true
 * @template T
 * @param {T[]} arr
